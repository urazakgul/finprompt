...
def main():
    ...
    st.caption("⚠️ Geliştirme geçici bir süre askıya alınmıştır. Mevcut sürümü kendi API anahtarınızla kullanabilirsiniz.")
    ...
    st.write("OpenAI API anahtarınızı aşağıdaki alana girdiğinizde tüm özellikleri tam erişimle kullanabilirsiniz. **Bu alan zorunludur.**")

    api_key, using_default = api_key_widget()

    if using_default or not api_key:
        st.warning("Lütfen kendi OpenAI API anahtarınızı giriniz.")
        return
